...
class DataStratifier:
    ...
    def split_data(
        ...
        # 1. Данные для стресс-теста
        stress_mask = (df_clean[self.target_col] == 0) & (df_clean[self.word_len_col] > self.stress_test_threshold)
        df_stress_test = df_clean[stress_mask]
        df_pool = df_clean[~stress_mask]

        logger.info(f"Выделено строк для Стресс-теста: {df_stress_test.shape[0]}")

        # Проверка на наличие данных для дальнейшего сплита
        if df_pool.empty:
            raise ValueError("Пул данны пуст после извлечения Стресс-теста.")

        # Выделение Train, Val, Test
        ...
        # Формирование Micro-Train сэмпла для быстрой проверки MVP
